...
`dimensions` 必须且仅包含 5 项，按 `方向契合`、`方法匹配`、`地域`、`学历目标`、`产出活跃` 输出。`score` 是 0-100 的信息性契合度，不是录取概率；`comment` 必须是中文非空一句话，信息不足时说明需要补充什么信息。

```json
{
  "professor_id": "p_001",
  "summary": "方向较契合，但仍需补充项目和成果信息。",
  "strengths": ["研究兴趣与导师方向有重合。"],
  "gaps": ["缺少可验证的项目或论文经历。"],
  "suggestions": ["优先阅读导师近三年论文，并整理相关项目经历。"],
  "dimensions": [
    { "label": "方向契合", "score": 82, "comment": "学生兴趣与导师研究方向有明显交集。" },
    { "label": "方法匹配", "score": 70, "comment": "已有专业背景可支撑入门，但需补充具体方法能力。" },
    { "label": "地域", "score": 60, "comment": "当前资料未体现地域偏好，建议补充目标城市或学校偏好。" },
    { "label": "学历目标", "score": 68, "comment": "目标阶段需要与导师招生类型进一步确认。" },
    { "label": "产出活跃", "score": 75, "comment": "导师方向活跃度可作为准备论文阅读和沟通材料的依据。" }
  ]
}
```
