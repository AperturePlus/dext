...
>
> 内容安全增量：匹配分析、套磁邮件与导师对比都必须经过违规内容过滤器；命中政治敏感、人身攻击、色情、暴力或导师攻击时硬拒答
>
...
- 明确禁止输出导师人格攻击、无来源负面指控或“避雷/垃圾”等攻击性结论——由共享内容政策的 `mentor_attack` 分类硬拒答。
...
```text
pin snapshot -> input policy review -> ProfessorDetail.fact_bundle -> FactBundle trimming
  -> await LLMGenerationPort.generate -> support validation -> CitationValidator
  -> SafetyGuard.inspect -> GenerationResult
```
...
- 内容政策命中（`content_policy_refusal` 及分类码）是 terminal error：返回 schema 兼容拒答对象或空 payload，不交付半清洗分析、邮件或对比结果。
...
- 内容政策命中：返回 `content_policy_refusal` error，claims/cited refs 清空，不返回被拒绝的输出片段。
- 缺少用户授权背景字段：返回 `insufficient_student_context` warning，不编造背景。

## 9. 验收标准

- 三个服务都通过共享 `LLMGenerationPort`/`CitationValidator`/`SafetyGuard.inspect_input`/`SafetyGuard.inspect`，不在 handler 中内联 prompt 或安全规则。
- 三个服务的 fake LLM 测试必须覆盖输入拒答与输出拒答：`content_policy_refusal` severity=`error`，claims/cited refs 为空，用户可见 payload 不含被拒绝文本。
